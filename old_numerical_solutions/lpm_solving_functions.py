...
import numpy as np
from data_prep_functions import load_extraction_rates,interpolate_forcer_linker,extraction_unit_convert
...
def solve_ode_pressure(f, t0, t1, dt, x0, pars):
    ...
    q=interpolate_forcer_linker(t,tv,qv)
...
def solve_ode_conc(f, t0, t1, dt, x0, pv, tv, pars):
    ...
    #get values for the linking variable
    p=interpolate_forcer_linker(t,tv,pv)
    #p = pressure_unit_convert(q)


    for k in range(nt):
		#define linking function q-VALUES FROM 1st LPM
        
        # k+1^th value of solution depends on k^th value of solution
		
        #find the slope at xk    
        dxdtk=f(t[k],x[k],p[k],*pars)
	    #increment solution by step size*derivative
        #to find a prediction of the solution at tk+h 
        xk1p=x[k]+dt*dxdtk
        
        #estimate the slope at xk1 using the prediction of xk1
        dxdtk1=f(t[k]+dt,xk1p,p[k],*pars)
