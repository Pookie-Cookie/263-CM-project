...
# define derivative function
def c_lpm(c,t,a,b,p0,p1,d,m0,csrc, testing = None, level=None):
...
def solve_c_lpm(t,a,b,p0,p1,d,m0,csrc, testing = None, extrapolate=None, level=None):
    ...
    for t0,t1 in zip(tc[:-1],tc[1:]):
        # predictor gradient
        dcdt1 = c_lpm(cm[-1], t0, a, b, p0, p1, d, m0, csrc, testing, level)
        # predictor step
        cp = cm[-1] + dcdt1*(t1-t0)             
        # corrector gradient
        dcdt2 = c_lpm(cp, t1, a, b, p0, p1, d, m0, csrc, testing, level)
        # corrector step
